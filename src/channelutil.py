...
from src import settings, stringutil, timeutil
    ...
    async def remove(self, hnm, channel):
        ...
        async for message in channel.history(limit=None):
            if (
                ...
                and message.content.startswith(f"- {hnm}")
                or message.content.startswith(f"- **{hnm}")
            ):
            ...
            if h == hnm:
                ...
                if day_split == "(****):":
                    if day >= 4:
                        ...
                        msg = hnm_list[hnm][:-7]
                        await channel.send(
                            f"- **{hnm}/{hq}** :rotating_light:{msg}{day_split[:3]}{day}{day_split[3:]} <t:{timestamp}:T> <t:{timestamp}:R>"
                        )
                    else:
                        msg = hnm_list[hnm][:-7]
                        await channel.send(
                            f"- {hnm} {msg}{day_split[:3]}{day}{day_split[3:]} <t:{timestamp}:T> <t:{timestamp}:R>"
                        )
                else:
                    msg = hnm_list[hnm]
                    await channel.send(
                        f"- {hnm} {msg} <t:{timestamp}:T> <t:{timestamp}:R>"
                    )
        ...
        for message in messages:
            ...
            if (
                any(
                    ...
                    for keyword in ["Jormungand", "Tiamat", "Vrtra"]
                )
                and time_diff > 86400 * 3
            ):
                continue
            if (
                not any(
...
class Tasks:
    ...
    async def start_channel_tasks(guild, channel_name, category, utc, hnm_name):
        channel = await guild.create_text_channel(
            channel_name, category=category, topic=f"<t:{utc}:T> <t:{utc}:R>"
        )
        ...
        wmtask = asyncio.create_task(
            Manager.window_manager(channel_name, category, guild)
        )
    ...
    async def restart_channel_tasks(
        guild, channel_name, category, time_diff, existing_channel
    ):
        if any(keyword in channel_name.lower() for keyword in ["jor", "vrt", "tia"]):
            if (
                time_diff >= 0
                and time_diff <= (24 * 3600) + 300
                and not settings.PROCESSEDLIST
            ):
                wmtask = asyncio.create_task(
                    Manager.window_manager(channel_name, category, guild)
                )
        ...
        try:
            for channel in channels:
                if (
                    ...
                    and channel_name in channel.name
                    and all(
                        keyword not in channel.name
                        for keyword in ["kv", "tia", "vrt", "jor"]
                    )
                ):
                    async for message in channel.history(limit=1, oldest_first=True):
                        ...
                        window_in_ten = stringutil.StringUtil.format_window_heading(
                            "Window opens in 20 Minutes x-in"
                        )
                    ...
                    isinstance(channel, discord.TextChannel)
                    and channel_name in channel.name
                    and any(keyword in channel.name for keyword in ["kv"])
                ):
                    async for message in channel.history(limit=1, oldest_first=True):
                        timestamp, dt = timeutil.Time.strip_timestamp(message)
                        dt = timeutil.Time.format_time(timestamp - (20 * 60))
                    ...
                    and channel_name in channel.name
                    and any(
                        keyword in channel.name for keyword in ["jor", "vrt", "tia"]
                    )
                ):
                    async for message in channel.history(limit=1, oldest_first=True):
                        timestamp, dt = timeutil.Time.strip_timestamp(message)
...
class Manager:
            ...
            for channel in channels:
                if (
                    ...
                    async for message in channel.history(limit=1, oldest_first=True):
                        ...
                        if time_diff <= -0 and not any(
                            keyword in channel.name for keyword in ["jor", "vrt", "tia"]
                        ):
                            await asyncio.sleep(sleep_time)
                            time_diff = timeutil.Time.now() - timestamp
                        elif gw_time <= -0 and any(
                            keyword in channel.name for keyword in ["jor", "vrt", "tia"]
                        ):
                            await asyncio.sleep(gw_sleep_time)
                            window = round(time_diff / 3600) + 1
                            first_window = stringutil.StringUtil.format_window_heading(
                                f"Window {window} in 5-Minutes x-in"
                            )
                            await channel.send(first_window)
                            await channel.send(
                                "```Remember a proper hold party(See Channel Pins) is required for DKP\nAll x-in's should be x-job, eg. x-blm```"
                            )
                        ...
                        if any(
                            ...
                            while time_diff >= -1200 and time_diff <= (
                                25 * 60 * 60
                            ):  # 25 windows * 60 minute * 60 seconds = total time in seconds
                                window = round(time_diff / 3600) + 1
                                window_open = time_diff + 300  # 300
                                window_close = time_diff - 60  # 360
                                if window_open % 3600 == 0:
                                    window_number_open = (
                                        stringutil.StringUtil.format_window_heading(
                                            f"Window {window} in 5-Minutes x-in"
                                        )
                                    )
                                ...
                                if window_close % 3600 == 0:
                                    window_number_close = (
                                        stringutil.StringUtil.format_window_heading(
                                            f"Window {window} closed no more x-in"
                                        )
                                    )
                            ...
                            poptask = asyncio.create_task(
                                Manager.dkp_review(
                                    guild, channel, channel_name, dt, timestamp
                                )
                            )
                            ...
                            if (
                                ...
                                kv_open = stringutil.StringUtil.format_window_heading(
                                    "King Vinegarroon Window is open"
                                )
                                ...
                                for task in existing_task:
                                    if (
                                        task.get_name() == f"wm-{channel_name}"
                                        or task.get_name() == f"wt-{channel_name}"
                                    ):
                                        task.cancel()
                                        log_print(
                                            f"Window Manager: WM/WT-Task for channel {channel_name} was completed."
                                        )
                        else:
                            while time_diff >= 0 and time_diff <= 3600:
                                if time_diff % 600 == 0:
                                    ...
                                    if "shi" in channel.name:
                                        nm_name = "Shikigami Weapon"
                                        instant_pop_nm_spawn = (
                                            stringutil.StringUtil.format_window_heading(
                                                f"{nm_name} has spawned"
                                            )
                                        )
                                        ...
                                        task_name = poptask.get_name()
                                        log_print(
                                            f"Pop: Task for {task_name} has been started."
                                        )
                                        settings.RUNNINGTASKS.append(task_name)
                                    elif "bs" in channel.name:
                                        nm_name = "Bloodsucker"
                                        instant_pop_nm_spawn = (
                                            stringutil.StringUtil.format_window_heading(
                                                f"{nm_name} has spawned"
                                        ...
                                        if window == 7:
                                            window_count = stringutil.StringUtil.format_window_heading(
                                                "POP"
                                            )
                                        else:
                                            window_count = stringutil.StringUtil.format_window_heading(
    ...
    async def dkp_review(guild, channel, channel_name, dt, utc):
        ...
        process_dict = {"id": channel.id, "utc": utc, "processed": True}
        ...
        for task in existing_task:
            if (
                ...
                task.cancel()
                log_print(
                    f"DKP Review: WM/WT-Task for channel {channel_name} was completed."
                )
        ...
        for task in existing_task:
            if (
                task.get_name() == f"pop-{channel_name}"
                or task.get_name() == f"close-{channel_name}"
            ):
                task.cancel()
                log_print(
                    f"DKP Review: Pop-Task for channel {channel_name} was completed."
                )
    ...
    async def open(ctx):
        ...
        for task in existing_task:
            if task.get_name() == f"wm-{ctx.channel.name}":
                ...
                task.cancel()
                log_print(
                    f"Open: Close-Task for channel {ctx.channel.name} was cancelled."
                )

        if not task_running:
            process_dict = {"id": ctx.channel.id, "utc": timestamp, "processed": False}
            ...
            msg = log_print(
                f"Open: {ctx.author.display_name} opened {ctx.channel.name}."
            )
            ...
            async for message in ctx.channel.history(limit=None, oldest_first=True):
                if message.content.startswith("- "):
                    ...
                    unix_target = int(dt.timestamp())
                    time_diff = unix_now - unix_target
                    await Tasks.restart_channel_tasks(
                        ctx.guild,
                        ctx.channel.name,
    ...
    async def close(ctx):
        ...
        for task in existing_task:
            if (
                task.get_name() == f"wm-{ctx.channel.name}"
                or task.get_name() == f"wt-{ctx.channel.name}"
            ):
                task.cancel()
        ...
        await LogPrint.print(ctx.bot, msg)
        closetask = asyncio.create_task(
            Manager.close_manager(ctx.channel.name, ctx.channel.category, ctx.guild)
        )
    ...
    async def pop(ctx, *linkshell):
        ...
        if isinstance(ctx.channel.name, discord.TextChannel):
            log_print(
                f"Pop: Channel {ctx.channel.name} is not a text channel. Ignoring."
            )
            return

        if (
            not ctx.channel.category
            or ctx.channel.category_id != settings.HNMCATEGORYID
        ):
            log_print(
                f"Pop: Channel {ctx.channel.name} is not in HNM Attendance category. Ignoring."
            )
            return
        ...
        for entry in settings.PROCESSEDLIST:
            if ctx.channel.id == entry["id"]:
                log_print(
                    f"Pop: {ctx.channel.name} has already been processed. Ignoring."
                )
                return await ctx.author.send(
                    "The !pop command has already been issued in this channel."
                )
        for entry in settings.EXTENDEDTASK:
            if ctx.channel.id == entry["id"]:
                log_print(
                    f"Pop: {ctx.channel.name} has been extend only !stable can close."
                )
                return await ctx.author.send(
                    "The !pop command doesn't work on extended camps. Please use !stable."
                )

        await ctx.message.delete()
        msg = log_print(
            f"Pop: {ctx.author.display_name} issued !pop command in {ctx.channel.name}."
        )
        await LogPrint.print(ctx.bot, msg)
        ...
        if any(keyword in channel_name_lower for keyword in ["faf", "beh", "ada"]):
            ...
            heading = stringutil.StringUtil.format_window_heading(
                f"POP: {window} | {linkshell}"
            )
        else:
        ...
        poptask = asyncio.create_task(
            Manager.dkp_review(ctx.guild, ctx.channel, ctx.channel.name, dt, timestamp)
        )
    ...
    async def camp_continue(ctx):
        ...
        msg = log_print(
            f"Continued: {ctx.author.display_name} extended camp {ctx.channel.name}."
        )
        await LogPrint.print(ctx.bot, msg)
        extendtask = asyncio.create_task(
            Manager.close_manager(ctx.channel.name, ctx.channel.category, ctx.guild)
        )
    ...
    async def enrage(ctx, window: int):
        ...
        while enrage:
            ...
            if (time_open - now) <= 0:
                enrage_close_msg = stringutil.StringUtil.format_window_heading(
                    f"Enrage {window} Closed"
                )
                await ctx.channel.send(enrage_close_msg)

                poptask = asyncio.create_task(
                    Manager.dkp_review(
                        ctx.guild, ctx.channel, ctx.channel.name, dt, timestamp
                    )
                )
                poptask.set_name(f"pop-{ctx.channel.name}")
                task_name = poptask.get_name()
    ...
    async def archive_channels(archive_category, archive_channel, category, option):
        ...
        if option == "delete":
            ...
            option == "all"
        ):  # 'all' argument will archive all channels in the category
            channels = category.channels
    ...
    async def sort_channels(ctx):
        ...
        msg = log_print(
            f"Sort Channels: {ctx.author.display_name} issued !sort_channels."
        )
        await LogPrint.print(ctx.bot, msg)
