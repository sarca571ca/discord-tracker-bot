...
from src import channelutil, settings, timeutil
...
class HandleHNM:
    ...
    async def find(self, ctx, cmd, day: int, mod: str, timestamp):
        if cmd in settings.FAFNIR:
            ...
            await HandleHNM.process(
                self.bot, ctx, "Shikigami Weapon", day, mod, timestamp
            )
        elif cmd in settings.SIM:
            await HandleHNM.process(self.bot, ctx, "Simurgh", day, mod, timestamp)
        elif cmd in settings.KV:
            await HandleHNM.process(
                self.bot, ctx, "King Vinegarroon", day, mod, timestamp
            )
        ...
        if hnm_window_end >= target_time:
            if hnm_time <= target_time:
                if any(
                    ...
                    for keyword in ["Fafnir", "Adamantoise", "Behemoth"]
                ):
                ...
                if existing_channel:
                    async for message in existing_channel.history(
                        limit=1, oldest_first=True
                    ):
                        channel_utc, _ = timeutil.Time.strip_timestamp(message)
                    if channel_utc == utc and category != hnm_category:
                        ...
                        for task in asyncio.all_tasks():
                            ...
                            if existing_channel.category == hnm_category:
                                await channelutil.Tasks.restart_channel_tasks(
                                    guild,
                                    channel_name,
                                    category,
                                    time_diff,
                        ...
                        if existing_channel:
                            ...
                            await channelutil.Tasks.start_channel_tasks(
                                guild, channel_name, category, utc, hnm_name
                            )
                else:
                    await channelutil.Tasks.start_channel_tasks(
                        guild, channel_name, category, utc, hnm_name
